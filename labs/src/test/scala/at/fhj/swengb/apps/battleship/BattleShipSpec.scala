...
class BattleShipSpec extends WordSpecLike {
  ...
  val validxPos = (0 until 4).map(x => BattlePos(x, 0)).toSet
  val validyPos = (0 until 4).map(y => BattlePos(0, y)).toSet
  ...
  "Battleship" should {
    ...
    "6 pos is illegal" in intercept[IllegalArgumentException](BattleShip("",Set(bp, bp, bp, bp, bp)))
